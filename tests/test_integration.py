...
class TestUserAgent:
    ...
    def test_custom_user_agent(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.get(f"{httpbin_url}/user-agent", user_agent=custom_ua)
        assert_valid_response(response, expected_status=200)
...
class TestHTTPMethods:
    ...
    def test_post_request(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.post(f"{httpbin_url}/post", json_data=payload)
        assert_valid_response(response, expected_status=200)
        ...
        if "json" in data:
            ...
            assert "POST-VAL" in data["data"], (
                f"Posted data should contain 'POST-VAL', got {data.get('data')}"
            )
    ...
    def test_put_request(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.put(f"{httpbin_url}/put", json_data=payload)
        assert_valid_response(response, expected_status=200)
        ...
        if "json" in data:
            ...
            assert "PUT-VAL" in data["data"], (
                f"PUT data should contain 'PUT-VAL', got {data.get('data')}"
            )
    ...
    def test_patch_request(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.patch(f"{httpbin_url}/patch", json_data=payload)
        assert_valid_response(response, expected_status=200)
        ...
        if "json" in data:
            ...
            assert "PATCH-VAL" in data["data"], (
                f"PATCH data should contain 'PATCH-VAL', got {data.get('data')}"
            )
    ...
    def test_delete_request(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.delete(f"{httpbin_url}/delete", json_data=payload)
        assert_valid_response(response, expected_status=200)
...
class TestHeaders:
    ...
    def test_custom_headers(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.get(f"{httpbin_url}/headers", headers=custom_headers)
        assert_valid_response(response, expected_status=200)
        ...
        assert auth_val == "Bearer someexampletoken", (
    ...
    def test_response_custom_header(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.get(f"{httpbin_url}/response-headers?MyCustomHeader=CustomValue")
        assert_valid_response(response, expected_status=200)
...
class TestCookies:
    ...
    def test_send_cookies(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.get(f"{httpbin_url}/cookies", cookies=cookies)
        assert_valid_response(response, expected_status=200)
...
class TestComplexScenarios:
    ...
    def test_post_with_headers_and_cookies(self, cycletls_client, httpbin_url):
        ...
        response = cycletls_client.post(
            f"{httpbin_url}/post", json_data=payload, headers=headers, cookies=cookies
        )
        assert_valid_response(response, expected_status=200)
        ...
        assert "session=abc123" in data["headers"]["Cookie"], (
            "Session cookie should be in Cookie header"
        )
    ...
    def test_all_methods_return_200(self, cycletls_client, httpbin_url, firefox_ja3):
        ...
        test_requests = [
            ...
            # IP
            {"method": "get", "url": f"{httpbin_url}/ip", "params": {}},
            # Response headers
            {
                "method": "get",
            ...
            # HTML
            {"method": "get", "url": f"{httpbin_url}/html", "params": {}},
            # Example.com (tests TLS extension 43)
            {"method": "get", "url": "https://example.com", "params": {}},
        ]
        ...
        for req in test_requests:
            ...
            if "/html" in url or "example.com" in url:
                assert isinstance(response.body, str), f"HTML response should be string for {url}"
